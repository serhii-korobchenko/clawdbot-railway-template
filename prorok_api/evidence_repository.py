...
def evidence_activity(conn: sqlite3.Connection, *, status: str = "active", window: str = "7d", now: datetime | None = None) -> dict[str, Any]:
    ...
    join_filter = ""; params: list[Any] = []
    if cutoff is not None:
        join_filter = " AND julianday(ei.created_at) >= julianday(?)"; params.append(_iso_utc(cutoff))
    params.append(status)
    rows = conn.execute(f"""SELECT e.event_id,e.title,e.status,COUNT(ei.evidence_id) AS evidence_count,SUM(CASE WHEN ei.direction='indicator' THEN 1 ELSE 0 END) AS indicator_count,SUM(CASE WHEN ei.direction='counterindicator' THEN 1 ELSE 0 END) AS counterindicator_count,SUM(CASE WHEN ei.direction='neutral' THEN 1 ELSE 0 END) AS neutral_count,MAX(strftime('%Y-%m-%dT%H:%M:%fZ',ei.created_at)) AS latest_evidence_at FROM events e LEFT JOIN evidence_items ei ON ei.event_id=e.event_id {join_filter} WHERE e.status=? GROUP BY e.event_id,e.title,e.status ORDER BY evidence_count DESC,latest_evidence_at DESC,e.title ASC""",params).fetchall()
...
def list_evidence(conn: sqlite3.Connection, *, event_id: str | None = None, direction: str | None = None, strength: str | None = None, source: str | None = None, q: str | None = None, sort: str = "newest", prorok_app: str | None = None) -> dict[str, Any]:
    clauses=[]; params=[]
    ...
    if prorok_app == "marked": clauses.append("COALESCE(pa.state,'unmarked') = 'marked'")
    elif prorok_app == "unmarked": clauses.append("COALESCE(pa.state,'unmarked') = 'unmarked'")
    ...
    rows=conn.execute(f"""SELECT ei.evidence_id,ei.run_id,ei.created_at,ei.direction,ei.strength,ei.summary,ei.relevance,ei.credibility,e.event_id,e.title AS event_title,e.status AS event_status,s.source_id,s.title AS source_title,s.domain,s.url,s.canonical_url,s.published_at,s.source_type,p.refresh_event_result_id,rer.refresh_id,d.decision_id,d.decision_type,d.baseline_probability,d.selected_probability,d.assessment_id,d.decided_at,em.evidence_assessment_decision_id AS manual_decision_id,em.baseline_probability AS manual_baseline_probability,em.selected_probability AS manual_selected_probability,em.assessment_id AS manual_assessment_id,em.decided_at AS manual_decided_at,COALESCE(pa.state,'unmarked') AS prorok_app_state,pa.source AS prorok_app_source,pa.actor AS prorok_app_actor,pa.changed_at AS prorok_app_changed_at FROM evidence_items ei JOIN events e ON e.event_id=ei.event_id JOIN sources s ON s.source_id=ei.source_id LEFT JOIN refresh_candidate_promotions p ON p.evidence_id=ei.evidence_id AND p.promotion_action='inserted' LEFT JOIN refresh_user_decisions d ON d.decision_id=p.decision_id LEFT JOIN refresh_event_results rer ON rer.refresh_event_result_id=p.refresh_event_result_id LEFT JOIN evidence_assessment_decisions em ON em.evidence_assessment_decision_id=(SELECT em2.evidence_assessment_decision_id FROM evidence_assessment_decisions em2 WHERE em2.evidence_id=ei.evidence_id ORDER BY em2.decided_at DESC,em2.evidence_assessment_decision_id DESC LIMIT 1) LEFT JOIN evidence_prorok_app_status_history pa ON pa.prorok_app_status_history_id=(SELECT pa2.prorok_app_status_history_id FROM evidence_prorok_app_status_history pa2 WHERE pa2.evidence_id=ei.evidence_id ORDER BY pa2.changed_at DESC,pa2.prorok_app_status_history_id DESC LIMIT 1) {where_sql} ORDER BY (s.published_at IS NULL) ASC,s.published_at {od},ei.evidence_id {od}""",params).fetchall()
...
def get_candidate_assessment_recommendation(conn: sqlite3.Connection, candidate_id: int) -> dict[str, Any] | None:
    ...
    result=dict(row); result["category_transition"]=bool(result["category_transition"]); return result
