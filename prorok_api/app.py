...
def create_app(settings: ApiSettings | None = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        validate_database(resolved_settings.db_path); app.state.settings = resolved_settings; yield
    app = FastAPI(title="PROROK Read-Only API", version="1.0.0", docs_url=None, redoc_url=None, openapi_url=None, lifespan=lifespan)

    @app.get("/healthz")
    def healthz():
        try:
            with readonly_connection(resolved_settings.db_path) as conn: conn.execute("SELECT 1").fetchone()
    ...
    @app.get("/api/v1/events", response_model=EventListResponse, dependencies=[Depends(require_api_token)])
    def events_list(status: EventStatusQuery | None = Query(default=None), q: str | None = Query(default=None, max_length=300)):
    ...
    def candidate_evidence_list(event_id: str | None = Query(default=None,max_length=200), direction: CandidateDirectionQuery | None = Query(default=None), strength: EvidenceStrengthQuery | None = Query(default=None), validation_state: CandidateValidationStateQuery | None = Query(default=None), source: str | None = Query(default=None,max_length=300), q: str | None = Query(default=None,max_length=300), sort: EvidenceSortQuery = Query(default="newest")):
        with readonly_connection(resolved_settings.db_path) as conn: return list_candidate_evidence(conn,event_id=event_id,direction=direction,strength=strength,validation_state=validation_state,source=source,q=q,sort=sort)
    ...
    def refresh_latest():
        with readonly_connection(resolved_settings.db_path) as conn: return get_latest_refresh(conn)

    @app.get("/api/v1/events/{event_id}/latest-recommendation", response_model=LatestRecommendationResponse, dependencies=[Depends(require_api_token)])
    def event_latest_recommendation(event_id: str):
        with readonly_connection(resolved_settings.db_path) as conn: result=get_latest_recommendation(conn,event_id)
        if result is None: raise HTTPException(status_code=404,detail="Event not found")
        return result

    @app.get("/api/v1/events/{event_id}", response_model=EventDetailResponse, dependencies=[Depends(require_api_token)])
    def event_detail(event_id: str):
        with readonly_connection(resolved_settings.db_path) as conn: result=get_event_detail(conn,event_id)
        if result is None: raise HTTPException(status_code=404,detail="Event not found")
        return result
    return app
