...
async def evidence_page(
    ...
    normalized_prorok_app = prorok_app or None
    ...
    try:
        ...
        if tab == "candidates":
            ...
            if lifecycle != "all":
                ...
                if lifecycle == "pending":
                    items = [
                        ...
                        and not item.get("review_decision_type")
                        and not item.get("promotion_action")
                        and not item.get("evidence_id")
                    ]
                else:
                    items = [
                        item for item in items
                        ...
                        or bool(item.get("review_decision_type"))
                        or bool(item.get("promotion_action"))
                        or bool(item.get("evidence_id"))
                    ]
            ...
            data = await request.app.state.prorok_api.list_evidence(
                ...
                q=normalized_q,
                sort=sort,
                prorok_app=normalized_prorok_app,
            )
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "lifecycle": lifecycle,
            "prorok_app": normalized_prorok_app or "",
